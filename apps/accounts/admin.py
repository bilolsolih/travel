...
class CustomUserAdmin(UserAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('phone_number', 'email', 'is_verified', 'is_active')}),
        ('Personal Info', {'fields': ('first_name', 'middle_name', 'last_name', 'gender', 'birthdate')}),
        ('Address', {'fields': ('region', 'district')}),
        ('Permissions', {'fields': ('is_staff', 'is_superuser')}),
    )
    add_fieldsets = (
        (None, {'classes': ('wide',), 'fields': ('phone_number', 'email', 'first_name', 'middle_name', 'last_name', 'password1', 'password2')}),
    )
